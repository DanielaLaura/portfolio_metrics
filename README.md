...
4. **Humans own the judgment calls.** Entity resolution, such as the FleetLink to Apex Freight rebrand, and the metric dictionary are hand-maintained seeds rather than LLM output. The extraction results themselves are declared as a dbt source, since dbt observes that data but does not own it.
...
- **Incremental runs**, processing only new PDFs and snapshotting the dbt models.
